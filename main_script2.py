...
def process_image(image_path, args, wd_caption, special_text, wd_features):
    """
    ...
    try:
        ...
        if "solo" in wd_features:
            tags_text = (
                f"{special_text}, {keep_tags}\n"
                f"{special_text}, {wd_caption}"
            )            
        else:
            more_detailed_caption, _ = run_example('<MORE_DETAILED_CAPTION>', image) 
            tags_text = (
                f"{special_text}, {keep_tags}, {more_detailed_caption}\n"
                f"{special_text}, {wd_caption}"
            )
...
def find_and_process_images(directory, args):
    ...
    special_texts, wd_captions, wd_features = {}, {}, {}
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for image_path in tqdm(image_paths, desc=f"處理圖片 {root}"):
            try:
                image = resize_image(image_path)
                rating, features, chars, _ = get_wd14_tags(image, character_threshold=0.6, general_threshold=0.27, drop_overlap=True, fmt=('rating', 'general', 'character', 'embedding'))      
                ...
                if aesthetic_tag != "":
                    aesthetic_tag = artisttag                    
                special_texts[image_path] = ", ".join(filter(None, [aesthetic_tag, special_text]))
                tags = list(set((wd_caption + boorutag).split(', ')))  # 去重
                random.shuffle(tags)
        ...
        for image_path in tqdm(image_paths, desc="打自然語言標"):
            try:
                ...
                process_image(image_path, args, wd_caption, special_texts[image_path], wd_features[image_path])
            except Exception as e:
                print(f"Failed to process image {image_path}: {e}")
                traceback.print_exc()
